...
const composerState={type:"",service:"",branchQuestion:false,branchAction:""},NL=String.fromCharCode(10);
...
let composerScreen=1;function showComposerScreen(screen){composerScreen=screen;whatsappComposerForm.querySelectorAll("[data-assistant-screen]").forEach(panel=>{panel.hidden=Number(panel.dataset.assistantScreen)!==screen});whatsappComposerForm.querySelectorAll(".composer-review-actions,.composer-submit,.composer-note").forEach(control=>{control.hidden=screen!==3});if(screen===3){whatsappComposerForm.elements.message.value=composerText();updateComposerPreview()}setTimeout(()=>{const panel=[...whatsappComposerForm.querySelectorAll("[data-assistant-screen]")].find(item=>Number(item.dataset.assistantScreen)===screen),target=panel&&panel.querySelector("input,select,textarea,[data-service],[data-composer-directions],[data-branch-question]");if(target)target.focus()},0)}function syncComposerNext(){const type=composerState.type,form=whatsappComposerForm,valid=type==="medicine"&&form.elements.medicine.value.trim()||type==="service"&&composerState.service||type==="product"&&form.elements.product.value.trim()||type==="branch"&&composerState.branchAction&&form.elements.branch.value||type==="other"&&form.elements.other.value.trim();form.querySelectorAll("[data-composer-next]").forEach(button=>{button.disabled=!valid})}function setComposerType(type){composerState.type=type;composerState.service="";composerState.branchQuestion=false;composerState.branchAction="";whatsappComposerForm.reset();whatsappComposerForm.elements.branch.value=state.branch;whatsappComposerForm.querySelectorAll("[data-help-type]").forEach(button=>{const selected=button.dataset.helpType===type;button.classList.toggle("selected",selected);button.setAttribute("aria-pressed",String(selected))});whatsappComposerForm.querySelectorAll("[data-detail]").forEach(field=>{field.hidden=field.dataset.detail!==type});whatsappComposerForm.querySelectorAll("[data-service]").forEach(button=>{button.classList.remove("selected");button.setAttribute("aria-pressed","false")});setComposerError("");showComposerScreen(type?2:1);syncComposerNext();if(type==="medicine")whatsappComposerForm.elements.medicine.focus();if(type==="product")whatsappComposerForm.elements.product.focus();if(type==="other")whatsappComposerForm.elements.other.focus()}function openWhatsAppComposer(context,detail){const openDialog=document.querySelector("dialog[open]");if(openDialog)openDialog.close();closeCart();setComposerType(context==="medicine"?"medicine":context==="service"?"service":context==="product"?"product":context==="branch"?"branch":context==="other"?"other":"");if(detail&&context==="product")whatsappComposerForm.elements.product.value=detail;if(detail&&context==="service"){const button=[...whatsappComposerForm.querySelectorAll("[data-service]")].find(item=>item.dataset.service===detail);if(button){composerState.service=detail;button.classList.add("selected");button.setAttribute("aria-pressed","true");whatsappComposerForm.querySelector("[data-composer-next=service]").disabled=false}}whatsappComposerForm.elements.branch.value=state.branch;whatsappComposerForm.elements.message.value=composerText();updateComposerPreview();syncComposerNext();whatsappComposer.showModal()}
...
whatsappComposerForm.querySelectorAll("[data-service]").forEach(button=>button.addEventListener("click",()=>{composerState.service=button.dataset.service;whatsappComposerForm.querySelector("[data-composer-next=service]").disabled=false;whatsappComposerForm.querySelectorAll("[data-service]").forEach(item=>{const selected=item===button;item.classList.toggle("selected",selected);item.setAttribute("aria-pressed",String(selected))});whatsappComposerForm.elements.message.value=composerText();updateComposerPreview();syncComposerNext()}));
whatsappComposerForm.querySelectorAll("input,textarea,select").forEach(field=>field.addEventListener("input",()=>{if(field.name==="branch"||field.name==="message")return;whatsappComposerForm.elements.message.value=composerText();updateComposerPreview();setComposerError("");syncComposerNext()}));
whatsappComposerForm.elements.branch.addEventListener("change",event=>{applyBranch(event.target.value,true);whatsappComposerForm.elements.message.value=composerText();updateComposerPreview();syncComposerNext()});
whatsappComposerForm.querySelector("[data-branch-question]").addEventListener("click",()=>{composerState.branchQuestion=true;composerState.branchAction="question";whatsappComposerForm.querySelector("[data-composer-next=branch]").disabled=false;whatsappComposerForm.elements.message.value=composerText();updateComposerPreview();syncComposerNext()});
whatsappComposerForm.querySelector("[data-composer-directions]").addEventListener("click",()=>{composerState.branchAction="directions";whatsappComposerForm.querySelector("[data-composer-next=branch]").disabled=false;whatsappComposerForm.elements.message.value=composerText();updateComposerPreview();syncComposerNext()});
function advanceComposer(kind){if(kind==="branch"&&composerState.branchAction==="directions"){const branch=BRANCHES[whatsappComposerForm.elements.branch.value];if(branch){window.open(branchDirectionsUrl(branch),"_blank","noopener");whatsappComposer.close()}return}showComposerScreen(3)}
whatsappComposerForm.querySelectorAll("[data-composer-next]").forEach(button=>button.addEventListener("click",()=>advanceComposer(button.dataset.composerNext)));whatsappComposerForm.querySelectorAll("[data-composer-back]").forEach(button=>button.addEventListener("click",()=>showComposerScreen(composerScreen===3?2:1)));
