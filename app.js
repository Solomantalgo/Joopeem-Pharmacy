...
const whatsappComposer=document.querySelector("#whatsapp-composer"),whatsappComposerForm=document.querySelector("#whatsapp-composer-form"),assistantStepContent=document.querySelector("#assistant-step-content"),assistantStepNavigation=document.querySelector("#assistant-step-navigation");
const assistantState={step:1,type:"",branch:state.branch,medicineName:"",service:"",productName:"",branchAction:"",customMessage:"",message:""},NL=String.fromCharCode(10),assistantServices=["Free Consultation","Blood Pressure Testing","Blood Glucose Testing","Malaria Testing","HIV PrEP Services","Medicine Advice & Counselling"];
function composerText(){const branch=assistantState.branch||state.branch;if(assistantState.type==="medicine")return assistantState.medicineName?"Hello Jopeem Pharmacy, I would like to check whether "+assistantState.medicineName+" is available at "+branch+"."+NL+NL+"Please confirm availability.":"Select Find a medicine and enter the medicine name.";if(assistantState.type==="service")return assistantState.service?"Hello Jopeem Pharmacy, I would like to ask about "+assistantState.service+" at "+branch+"."+NL+NL+"Please let me know about availability and any requirements.":"Select a health service to continue.";if(assistantState.type==="product")return assistantState.productName?"Hello Jopeem Pharmacy, I would like to check availability of "+assistantState.productName+" at "+branch+"."+NL+NL+"Please confirm whether it is currently available.":"Enter the product or item name to continue.";if(assistantState.type==="branch"&&assistantState.branchAction==="question")return "Hello Jopeem Pharmacy, I would like information about your "+branch+" branch, including opening hours and contact details.";if(assistantState.type==="other")return assistantState.customMessage?"Hello Jopeem Pharmacy, I would like some help with the following:"+NL+NL+assistantState.customMessage:"Tell us briefly how we can help.";return "Hello Jopeem Pharmacy, I would like some assistance."}
function updateComposerPreview(){const preview=assistantStepContent.querySelector("[data-message-preview]"),count=assistantStepContent.querySelector("[data-message-count]");if(preview)preview.textContent=assistantState.message;if(count)count.textContent=assistantState.message.length}
function setComposerError(message){let error=assistantStepContent.querySelector("[data-composer-error]");if(!error){error=document.createElement("p");error.className="composer-error";error.dataset.composerError="";assistantStepContent.appendChild(error)}error.textContent=message||"";error.hidden=!message}
function assistantValid(){return assistantState.type==="medicine"&&assistantState.medicineName||assistantState.type==="service"&&assistantState.service||assistantState.type==="product"&&assistantState.productName||assistantState.type==="branch"&&assistantState.branch&&assistantState.branchAction||assistantState.type==="other"&&assistantState.customMessage}
function renderAssistantStep(){let html="";if(assistantState.step===1){html="<fieldset class=\"composer-step\"><legend data-i18n=\"assistant.step1\">Step 1 of 3 · What do you need help with?</legend><div class=\"composer-help-options\" role=\"group\" aria-label=\"Help type\"><button type=\"button\" class=\"composer-help-option\" data-help-type=\"medicine\"><span aria-hidden=\"true\">⌕</span><b data-i18n=\"assistant.medicine\">Find a medicine</b></button><button type=\"button\" class=\"composer-help-option\" data-help-type=\"service\"><span aria-hidden=\"true\">＋</span><b data-i18n=\"assistant.service\">Ask about a health service</b></button><button type=\"button\" class=\"composer-help-option\" data-help-type=\"product\"><span aria-hidden=\"true\">□</span><b data-i18n=\"assistant.product\">Check product availability</b></button><button type=\"button\" class=\"composer-help-option\" data-help-type=\"branch\"><span aria-hidden=\"true\">⌖</span><b data-i18n=\"assistant.branch\">Branch &amp; directions</b></button><button type=\"button\" class=\"composer-help-option\" data-help-type=\"other\"><span aria-hidden=\"true\">⋯</span><b data-i18n=\"assistant.other\">Something else</b></button></div></fieldset>"}else if(assistantState.step===2){html="<fieldset class=\"composer-step\"><legend data-i18n=\"assistant.step2\">Step 2 of 3 · Relevant details</legend>";if(assistantState.type==="medicine")html+="<label class=\"composer-field-label\" data-i18n=\"assistant.medicineName\">Medicine name<input data-assistant-field=\"medicine\" autocomplete=\"off\" placeholder=\"e.g. Paracetamol\"></label>";if(assistantState.type==="service")html+="<span class=\"composer-field-label\" data-i18n=\"assistant.healthService\">Health service</span><div class=\"composer-service-options\" role=\"group\" aria-label=\"Health services\">"+assistantServices.map(service=>"<button type=\"button\" data-service=\""+service+"\" aria-pressed=\""+String(assistantState.service===service)+"\" class=\""+(assistantState.service===service?"selected":"")+"\">"+service+"</button>").join("")+"</div>";if(assistantState.type==="product")html+="<label class=\"composer-field-label\" data-i18n=\"assistant.productName\">Product or item name<input data-assistant-field=\"product\" autocomplete=\"off\" placeholder=\"e.g. body lotion, glucose strips, thermometer\"></label>";if(assistantState.type==="branch")html+="<label class=\"composer-branch\">Preferred branch<select data-assistant-field=\"branch\"><option>Nyanama Trading Centre</option><option>Lebron Shopping Complex, Nalumunye</option></select></label><div class=\"composer-branch-actions\"><button type=\"button\" class=\"button button-outline\" data-composer-directions data-i18n=\"assistant.directions\">Get directions</button><button type=\"button\" class=\"text-button\" data-branch-question data-i18n=\"assistant.branchQuestion\">Ask about branch hours/contact</button></div>";if(assistantState.type==="other")html+="<label class=\"composer-field-label\" data-i18n=\"assistant.customPrompt\">Tell us briefly how we can help.<textarea data-assistant-field=\"other\" rows=\"4\" placeholder=\"Tell us briefly how we can help.\"></textarea></label>";html+="</fieldset>"}else html="<section class=\"composer-step composer-review\"><p class=\"composer-step-label\" data-i18n=\"assistant.step3\">Step 3 of 3 · Review your message</p><p class=\"composer-review-branch\"><b data-i18n=\"assistant.preferredBranch\">Preferred branch</b><span>"+assistantState.branch+"</span></p><label class=\"composer-message\" data-i18n=\"assistant.message\">Your message<textarea data-assistant-field=\"message\" rows=\"4\" maxlength=\"500\"></textarea><small><span data-message-count>0</span>/500</small></label><div class=\"composer-preview\" aria-live=\"polite\"><span data-i18n=\"assistant.preview\">Preview</span><p data-message-preview></p></div><small class=\"composer-note\" data-i18n=\"assistant.safety\">For medical advice, treatment decisions or urgent health concerns, speak directly with our pharmacy team or an appropriate healthcare professional.</small></section>";assistantStepContent.innerHTML=html;if(window.JopeemI18n)window.JopeemI18n.apply(assistantStepContent);const medicine=assistantStepContent.querySelector("[data-assistant-field=medicine]"),product=assistantStepContent.querySelector("[data-assistant-field=product]"),other=assistantStepContent.querySelector("[data-assistant-field=other]"),branch=assistantStepContent.querySelector("[data-assistant-field=branch]"),message=assistantStepContent.querySelector("[data-assistant-field=message]");if(medicine)medicine.value=assistantState.medicineName;if(product)product.value=assistantState.productName;if(other)other.value=assistantState.customMessage;if(branch)branch.value=assistantState.branch;if(message)message.value=assistantState.message;renderAssistantNavigation();setTimeout(()=>{const target=assistantState.step===3?message:assistantStepContent.querySelector("input,select,textarea,[data-service],[data-help-type]");if(target)target.focus()},0)}
function renderAssistantNavigation(){assistantStepNavigation.innerHTML="";if(assistantState.step===2)assistantStepNavigation.innerHTML="<button type=\"button\" class=\"button button-outline\" data-assistant-back><span data-i18n=\"assistant.back\">Back</span></button><button type=\"button\" class=\"composer-next\" data-assistant-next "+(assistantValid()?"":"disabled")+"><span data-i18n=\"assistant.next\">Next</span> →</button>";if(assistantState.step===3)assistantStepNavigation.innerHTML="<button type=\"button\" class=\"button button-outline\" data-assistant-back><span data-i18n=\"assistant.back\">Back</span></button><button class=\"button button-primary composer-submit\" type=\"submit\"><span data-i18n=\"assistant.continue\">Continue to WhatsApp</span> →</button>";if(window.JopeemI18n)window.JopeemI18n.apply(assistantStepNavigation)}
function setComposerType(type,detail){assistantState.step=type?2:1;assistantState.type=type;assistantState.branch=state.branch;assistantState.medicineName="";assistantState.service="";assistantState.productName=detail&&type==="product"?detail:"";assistantState.branchAction="";assistantState.customMessage="";assistantState.message="";renderAssistantStep()}
function openWhatsAppComposer(context,detail){const openDialog=document.querySelector("dialog[open]");if(openDialog)openDialog.close();closeCart();setComposerType(context==="medicine"?"medicine":context==="service"?"service":context==="product"?"product":context==="branch"?"branch":context==="other"?"other":"",detail);if(detail&&context==="service"&&assistantServices.includes(detail))assistantState.service=detail;renderAssistantStep();whatsappComposer.showModal()}
assistantStepContent.addEventListener("click",event=>{const type=event.target.closest("[data-help-type]"),service=event.target.closest("[data-service]"),directions=event.target.closest("[data-composer-directions]"),question=event.target.closest("[data-branch-question]");if(type){setComposerType(type.dataset.helpType);return}if(service){assistantState.service=service.dataset.service;assistantStepContent.querySelectorAll("[data-service]").forEach(item=>{const selected=item===service;item.classList.toggle("selected",selected);item.setAttribute("aria-pressed",String(selected))});renderAssistantNavigation();return}if(directions){assistantState.branchAction="directions";renderAssistantNavigation();return}if(question){assistantState.branchAction="question";renderAssistantNavigation()}});assistantStepContent.addEventListener("input",event=>{const field=event.target.dataset.assistantField;if(field==="medicine")assistantState.medicineName=event.target.value.trim();if(field==="product")assistantState.productName=event.target.value.trim();if(field==="other")assistantState.customMessage=event.target.value.trim();if(field==="message")assistantState.message=event.target.value;setComposerError("");if(assistantState.step===2)renderAssistantNavigation();if(assistantState.step===3)updateComposerPreview()});assistantStepContent.addEventListener("change",event=>{if(event.target.dataset.assistantField==="branch"){assistantState.branch=event.target.value;applyBranch(assistantState.branch,true);renderAssistantStep()}});assistantStepNavigation.addEventListener("click",event=>{if(event.target.closest("[data-assistant-back]")){assistantState.step=assistantState.step===3?2:1;renderAssistantStep();return}if(event.target.closest("[data-assistant-next]")){if(!assistantValid())return;if(assistantState.type==="branch"&&assistantState.branchAction==="directions"){const branch=BRANCHES[assistantState.branch];if(branch){window.open(branchDirectionsUrl(branch),"_blank","noopener");whatsappComposer.close()}return}assistantState.message=composerText();assistantState.step=3;renderAssistantStep()}});whatsappComposerForm.addEventListener("submit",event=>{event.preventDefault();if(assistantState.step!==3)return;const message=String(assistantState.message||"").trim();if(!message){setComposerError("Please enter a message.");return}whatsappComposer.close();window.open(wa(message),"_blank","noopener")});const contactFab=$(".contact-fab"),contactMenu=$("#contact-menu"),contactBackdrop=$(".contact-panel-backdrop");
